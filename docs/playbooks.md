...
| `notify` | `channel` (default `all`; must be `all`, `discord`, `email`, `telegram`, or `webhook`) | Sends an alert through one of the configured channels (`docs/alerts.md`) as a scripted step, in addition to the baseline alert the system already fires on a DDoS verdict, not a replacement for it. `channel` actually restricts delivery to just that one channel; a value outside the closed set is rejected at save time (`playbooks.validate_definition()`). |
