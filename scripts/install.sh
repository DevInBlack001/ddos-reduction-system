...
#   --training-csv <PATH>    CSV to retrain all three models
...
# CSV to retrain all three models against. No default: the
...
EOF
        ...
        # retrained for the feature to keep producing output; the Isolation
        # Forest is retrained in the same job so its contamination boundary
        # doesn't go stale against live traffic either). Opt-in only.
        if [[ -n "$TRAINING_CSV" ]]; then
            cat > "$SERVICE_DIR/ddos-stage2-retrain.service" << EOF
# =============================================================================
# ddos-stage2-retrain.service, periodic retrain of all three models
# Generated by install.sh on $(date -u +"%Y-%m-%dT%H:%M:%SZ")
# =============================================================================
...
Environment="IF_MODEL_PATH=$STAGE2_STATE_DIR/ddos_if_model.joblib"
Environment="SECOND_MODEL_PATH=$STAGE2_STATE_DIR/ddos_gb_model.joblib"
ExecStart=/bin/bash -c 'source "$STAGE2_INSTALL_DIR/venv/bin/activate" && python3 train.py "$TRAINING_CSV" && python3 train_isolation_forest.py "$TRAINING_CSV" && exec python3 train_second_model.py "$TRAINING_CSV"'
Nice=10
CPUWeight=20
IOSchedulingClass=idle
EOF
...
# =============================================================================
# ddos-stage2-retrain.timer, runs the periodic model retrain job. All three
# models must be retrained periodically: the RF and second model because
# auto_label.py's freshness safeguard will permanently refuse to auto-label
# any newly captured traffic once they predate it, and the Isolation Forest
# because a stale one keeps scoring live traffic against a contamination
# rate and boundary selected from an increasingly old capture, which is what
# let it drift toward over-flagging real, benign live traffic as Anomalous.
# Generated by install.sh on $(date -u +"%Y-%m-%dT%H:%M:%SZ")
# =============================================================================
...
WantedBy=timers.target
EOF
            success "Systemd timer created for periodic retraining of all three models."
            RETRAIN_TIMER_INSTALLED=true
        else
            info "Skipping the retrain timer: no --training-csv given. Pass one to enable periodic retraining of all three models."
            RETRAIN_TIMER_INSTALLED=false
        fi
    fi

    systemctl daemon-reload
    success "Systemd services installed: ddos-stage1.service, ddos-stage2.service, ddos-stage2-auto-label.timer"
    if [[ "$CAPTURE_MODE" == "kernel" ]]; then
