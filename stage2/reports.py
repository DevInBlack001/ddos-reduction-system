...
class _ReportCanvas(pdfcanvas.Canvas):
    ...
    def _draw_chrome(self, page_count):
        ...
        self.drawString(36, 28, f"SHIELD Gateway — CONFIDENTIAL — {self._report_id}")
...
def export_pdf(payload: PdfReportPayload, request: Request):
    ...
    try:
        report_id = f"SGW-{uuid.uuid4().hex[:12].upper()}"
        ...
        meta_data = [
            ["REPORT ID", report_id, "GENERATED", time.strftime('%Y-%m-%d %H:%M:%S')],
        ...
        elements.append(Paragraph(
            f"This report summarizes the operational and security posture of the SHIELD Gateway "
